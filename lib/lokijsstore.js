 ...
 * (2) { lokijsstore: {disablesaveatwrite: boolean}} disables automatic write to 
 * disk whenever changes to the database occur. Recommended for more intense 
 * workloads. Should only be set to true when LokiJS's autosave is set to 
 * true. Default: false
 * @constructor
 */
function LokiJSStore(file, options) {
	...
	this._disablesaveatwrite = false;
	if(this._options.lokijsstore) {
		...
		if(this._options.lokijsstore.disablesaveatwrite) {
			this._disablesaveatwrite = true;
		}
...
LokiJSStore.prototype._connect = function(callback) {
	var self = this;
	if(!self._db) {
		self._db = new Loki(self._file, self._options);
	} 
	callback(self._db);
}
...
/**
 * Private method to save the database if needed
 * @private
 */
LokiJSStore.prototype._saveDatabase = function(callback) {
	var self = this;

	if(self._disablesaveatwrite) {
		callback();
	} else {
		self._connect(function(db) {
